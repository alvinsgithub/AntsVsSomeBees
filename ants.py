...
Name: Alvin Yau, Mandy Huang
Login: cs61a-qe, cs61a-qi
TA: Hamilton Nguyen
Section: 18
"""
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if random_or_none(self.place.bees) != None:
            ...
            while random_or_none(cur_place.bees) == None and cur_place != hive:
...
class AntColony(object):
    ...
    def remove_ant(self, place_name):
        ...
        if type(place.ant) == BodyguardAnt:
            inner = place.ant.ant
            if type(inner) == QueenAnt:
                return
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        for i in range(0, self.min_range):
            cur_place = cur_place.entrance
            #print("min_range:", cur_place)
        while random_or_none(cur_place.bees) == None and cur_place != hive:
            cur_place = cur_place.entrance
            #print("checking none:", cur_place)
        if cur_place == hive:
            #print("is hive")
            return None
        #print("final place:", cur_place)
        return random_or_none(cur_place.bees)
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        for i in range(0, self.max_range):
            if random_or_none(cur_place.bees) == None and cur_place != hive:
                cur_place = cur_place.entrance
        if cur_place == hive:
                return None
        return random_or_none(cur_place.bees)
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        "*** YOUR CODE HERE ***"
        """eat the bee, set digesting to time_to_digest"""
        bee.reduce_armor(bee.armor)
        self.digesting = self.time_to_digest

    def action(self, colony):
        "*** YOUR CODE HERE ***"
        """if digesting, decrement self.digesting; otherwise, eat_bee"""
        if self.digesting != 0:
            self.digesting -= 1
        else:
            if random_or_none(self.place.bees) != None:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, colony):
        ...
        #if self.double != True: 
        while place != None: 
            if place.ant != None and place.ant.double == False: 
...
class QueenPlace(object): 
    ...
    def bees(self): 
        beeslst = []
        beeslst.extend(self.colony_queen.bees)
        beeslst.extend(self.place.bees)
        #print('Beelist: ', beeslst)
        return beeslst
...
def apply_effect(effect, bee, duration):
    ...
    def effect2(colony):
        nonlocal duration
        if duration == 0:
            #print("changing back to normal:")
            orig_action(colony)
        else:
            #print("action with effect")
